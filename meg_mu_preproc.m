...
n = input(2); % participant's id number
o = input(3); % group number
first = 200;
keys = 300;
rhys = 400;
meters = 500;
...
%% TRIAL DEFINITION
labels_concat = cell(1,1); % dummy variable for concatenating labels
for h = 1:length(serieNames)
    if ~exist(fullfile(savePath, groupNames{o}, serieNames{h}, extName), 'dir')
        mkdir(fullfile(savePath, groupNames{o}, serieNames{h}, extName));
    end
    %%% Define trial structure for ALL trials 
    cfg = [];
    cfg.dataset = fullfile(savePath, groupNames{o}, serieNames{h}, 'tsss', sprintf('%04d_%s_tsss_mc.fif',n,serieNames{h}));
    ...
    if ~isempty(spurious)
        cfg.trl(spurious,:) = [];
        save(fullfile(savePath, groupNames{o}, serieNames{h}, extName, sprintf('index_%04d_%s',n,serieNames{h})),'spurious','cfg');
    end
    ...
    if ~strcmp(serieNames{h},'ns')
        ...
        stds3_clean = cfg.trl(stds3, 4) < 20;
    end
...
%     cfg.headerfile = fullfile(savePath, groupNames{o}, serieNames{h}, 'tsss', sprintf('%04d_%s_tsss_mc.fif',n,serieNames{h}));
    ...
    labels_concat = [labels_concat; labels];
    conds{h} = data;
end
labels_concat(1) = []; % deleting first empty cell

%% APPEND BLUE/GREEN/RED/YELLOW
...
%% AVERAGING
% loading option for only running the averaging part of this function
% load(fullfile(savePath, groupNames{o}, serieNames{h}, extName, sprintf('%04d_preprocessed.mat',n)))

% save(fullfile(savePath, groupNames{o}, serieNames{h}, extName, sprintf('%04d_preprocessed.mat',n)), 'data', '-v7.3')

%% TIMELOCK ANALYSIS AND IMPLICITLY SELECTING DATA FOR SINGLE TRIAL ANALYSIS
% This probably needs to be done separately for different conditions...
...
for j = 1:length(unique(labels_concat)) % length(condList)
   ...
   cfg.trials = find(match_str(labels_concat, condList{j}));    
    
   cfg_old = cfg;
   cfg_old.keeptrials = 'no';
   data_struct.(condNames{j}) = ft_timelockanalysis(cfg, data);
   data_struct_tlck.(sprintf('%s_old',condNames{j})) = ft_timelockanalysis(cfg_old, data);
end

save(fullfile(savePath, groupNames{o}, serieNames{h}, extName, sprintf('%04d_evoked.mat',n)),'data_struct_tlck');
save(fullfile(savePath, groupNames{o}, serieNames{h}, extName, sprintf('%04d_preprocessed.mat',n)),'data_struct', '-v7.3');
